...
插入模式下的操作
"<C-h>"删除前一个字符，同delete键
"<C-w>"删除前一个单词
"<C-u>"删至行首
"<C-r>{register}"粘贴对应的寄存器中的文本到当前的光标
...
第十六章：ctags建立索引，用其浏览源码

使用ctags需要先安装，再设置$PATH,确保/usr/local/bin比/usr/bin优先
...
第十八章：用grep、vimgrep和其它工具对整个工程进行查找
